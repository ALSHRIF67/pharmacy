    ...
    <div class="min-h-screen">
        ...
        <nav class="max-w-7xl mx-auto px-6 py-8 flex justify-between items-center">
            ...
            <div class="flex items-center space-x-4 space-x-reverse">
                <a href="{{ route('pos.index') }}" class="bg-emerald-600 text-white px-6 py-3 rounded-xl font-bold hover:bg-emerald-700 transition btn-shadow">ابدأ البيع الآن</a>
            </div>
        </nav>
        ...
        <div class="max-w-7xl mx-auto px-6 pt-16 pb-24 grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            <div>
                ...
                <div class="grid grid-cols-3 gap-8">
                    <div>
                        <div class="text-3xl font-black text-slate-900">{{ number_format($stats['today_sales'], 0) }}</div>
                        <div class="text-sm text-slate-500 font-bold">مبيعات اليوم</div>
                    </div>
                    <div>
                        <div class="text-3xl font-black text-slate-900">{{ $stats['total_products'] }}</div>
                        <div class="text-sm text-slate-500 font-bold">إجمالي الأصناف</div>
                    </div>
                    <div>
                        <div class="text-3xl font-black text-rose-500">{{ $stats['low_stock_count'] }}</div>
        ...
        <div class="max-w-7xl mx-auto px-6 pb-24">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                <!-- POS -->
                <a href="{{ route('pos.index') }}" class="glass-card p-8 rounded-[2rem] text-right group">
                ...
                <!-- Products -->
                <a href="{{ route('products.index') }}" class="glass-card p-8 rounded-[2rem] text-right group">
                ...
                <!-- Purchases -->
                <a href="{{ route('purchases.index') }}" class="glass-card p-8 rounded-[2rem] text-right group">
                ...
                <!-- Reports -->
                <a href="{{ route('reports.index') }}" class="glass-card p-8 rounded-[2rem] text-right group">
        ...
        <footer class="max-w-7xl mx-auto px-6 py-12 border-t border-slate-200">
            <div class="flex flex-col md:flex-row justify-between items-center text-slate-400 text-sm font-bold">
                <div>&copy; {{ date('Y') }} فارما هب - جميع الحقوق محفوظة</div>
                <div class="mt-4 md:mt-0 flex space-x-6 space-x-reverse">
                    <a href="{{ route('sync.index') }}" class="hover:text-emerald-600 transition">حالة المزامنة</a>
                    <a href="{{ route('customers.index') }}" class="hover:text-emerald-600 transition">العملاء</a>
                    <a href="{{ route('suppliers.index') }}" class="hover:text-emerald-600 transition">الموردين</a>
                </div>
            </div>
        </footer>
    </div>
