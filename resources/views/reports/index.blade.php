    ...
    <table class="w-full border-collapse">
        ...
        <tbody>
            @foreach($sales as $sale)
                <tr class="border-b">
                    <td class="p-3 text-sm text-gray-700">{{ $sale->id }}</td>
                    <td class="p-3 text-sm text-emerald-600 font-bold">{{ number_format($sale->total_price, 2) }}</td>
                    <td class="p-3 text-sm text-gray-700">{{ $sale->created_at }}</td>
                    <td class="p-3 text-sm">
                        <a href="{{ route('sales.show', $sale->id) }}" class="text-blue-500 hover:underline">عرض</a>
                        <a href="{{ route('sales.edit', $sale->id) }}" class="text-yellow-500 hover:underline">تعديل</a>
